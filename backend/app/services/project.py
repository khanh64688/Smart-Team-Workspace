...
class ProjectService:
    ...
    def require_project(self, project_id: uuid.UUID) -> Project:
        ...
        if not project:
            raise api_error(404, "PROJECT_NOT_FOUND", "Không tìm thấy dự án.")
        return project
    ...
    def update(self, project_id: uuid.UUID, payload: ProjectUpdate, actor: User):
        project = self.require_project(project_id)
        self.require_manager(project_id, actor)
    ...
    def soft_delete(self, project_id: uuid.UUID, actor: User):
        if actor.role != UserRole.ADMIN:
            raise api_error(403, "ADMIN_REQUIRED", "Chỉ quản trị viên được xoá dự án.")
        project = self.require_project(project_id)
    ...
    def add_member(self, project_id: uuid.UUID, payload: MemberCreate, actor: User):
        ...
        if self.require_project(project_id).status == ProjectStatus.CLOSED:
            raise api_error(409, "PROJECT_CLOSED", "Dự án đã đóng và chỉ có thể đọc.")
        if not self.db.get(User, payload.user_id):
            raise api_error(404, "USER_NOT_FOUND", "Không tìm thấy người dùng.")
        if payload.project_role == ProjectRole.OWNER:
            raise api_error(400, "OWNER_TRANSFER_REQUIRED", "Không thể thêm trực tiếp vai trò OWNER.")
        ...
        try:
            ...
            self.db.rollback()
            raise api_error(409, "PROJECT_MEMBER_EXISTS", "Người dùng đã là thành viên dự án.") from err
