import uuid
from typing import Annotated

from fastapi import (
    APIRouter,
    Depends,
    Query,
    Response,
)

from app.core.deps import (
    CurrentUser,
    DbSession,
    ProjectAdminOrPmUser,
    ProjectManagerUser,
    ProjectMemberUser,
    ProjectOwnerUser,
    require_role,
)
from app.models import (
    ProjectStatus,
    User,
    UserRole,
)
from app.repositories import ProjectRepository
from app.schemas.project import (
    MemberConfigUpdate,
    MemberCreate,
    MemberOut,
    MemberRoleUpdate,
    ProjectCreate,
    ProjectOut,
    ProjectPage,
    ProjectUpdate,
)
from app.services import ProjectService


router = APIRouter(
    prefix="/projects",
    tags=["Projects"],
)


AdminUser = Annotated[
    User,
    Depends(
        require_role(
            UserRole.ADMIN,
        )
    ),
]


def service(
    db: DbSession,
) -> ProjectService:
    return ProjectService(db)


def member_out(member) -> MemberOut:
    return MemberOut(
        user_id=member.user_id,
        full_name=member.user.full_name,
        email=member.user.email,
        project_role=member.project_role,
        can_config=member.can_config,
        joined_at=member.joined_at,
    )


@router.post(
    "",
    response_model=ProjectOut,
    status_code=201,
)
def create_project(
    payload: ProjectCreate,
    actor: ProjectAdminOrPmUser,
    svc: ProjectService = Depends(service),
):
    return svc.create(
        payload,
        actor,
    )


@router.get(
    "",
    response_model=ProjectPage,
)
def list_projects(
    actor: CurrentUser,
    db: DbSession,
    q: str | None = None,
    project_status: ProjectStatus | None = Query(
        None,
        alias="status",
    ),
    page: int = Query(
        1,
        ge=1,
    ),
    size: int = Query(
        20,
        ge=1,
        le=100,
    ),
    sort: str = "-created_at",
):
    data, total = ProjectRepository(db).list(
        actor.id,
        actor.role == UserRole.ADMIN,
        q,
        project_status,
        page,
        size,
        sort,
    )

    return {
        "data": data,
        "meta": {
            "page": page,
            "size": size,
            "total": total,
        },
    }


@router.get(
    "/{project_id}",
    response_model=ProjectOut,
)
def get_project(
    project_id: uuid.UUID,
    actor: ProjectMemberUser,
    svc: ProjectService = Depends(service),
):
    project = svc.require_project(project_id)

    svc.require_member(
        project_id,
        actor,
    )

    return project


@router.put(
    "/{project_id}",
    response_model=ProjectOut,
)
def update_project(
    project_id: uuid.UUID,
    payload: ProjectUpdate,
    actor: ProjectManagerUser,
    svc: ProjectService = Depends(service),
):
    return svc.update(
        project_id,
        payload,
        actor,
    )


@router.patch(
    "/{project_id}/close",
    response_model=ProjectOut,
)
def close_project(
    project_id: uuid.UUID,
    actor: ProjectOwnerUser,
    svc: ProjectService = Depends(service),
):
    return svc.close(
        project_id,
        actor,
    )


@router.patch(
    "/{project_id}/reopen",
    response_model=ProjectOut,
)
def reopen_project(
    project_id: uuid.UUID,
    actor: ProjectOwnerUser,
    svc: ProjectService = Depends(service),
):
    return svc.reopen(
        project_id,
        actor,
    )


@router.delete(
    "/{project_id}",
    status_code=204,
)
def delete_project(
    project_id: uuid.UUID,
    actor: AdminUser,
    svc: ProjectService = Depends(service),
):
    svc.soft_delete(
        project_id,
        actor,
    )

    return Response(
        status_code=204,
    )


@router.get(
    "/{project_id}/members",
    response_model=list[MemberOut],
)
def list_members(
    project_id: uuid.UUID,
    actor: ProjectMemberUser,
    svc: ProjectService = Depends(service),
):
    svc.require_member(
        project_id,
        actor,
    )

    members = svc.repo.members(project_id)

    return [
        member_out(member)
        for member in members
    ]


@router.post(
    "/{project_id}/members",
    response_model=MemberOut,
    status_code=201,
)
def add_member(
    project_id: uuid.UUID,
    payload: MemberCreate,
    actor: ProjectManagerUser,
    svc: ProjectService = Depends(service),
):
    member = svc.add_member(
        project_id,
        payload,
        actor,
    )

    return member_out(member)


@router.patch(
    "/{project_id}/members/{user_id}",
    response_model=MemberOut,
)
def change_member_role(
    project_id: uuid.UUID,
    user_id: uuid.UUID,
    payload: MemberRoleUpdate,
    actor: ProjectManagerUser,
    svc: ProjectService = Depends(service),
):
    member = svc.change_role(
        project_id,
        user_id,
        payload.project_role,
        actor,
    )

    return member_out(member)


@router.delete(
    "/{project_id}/members/me",
    status_code=204,
)
def leave_project(
    project_id: uuid.UUID,
    actor: ProjectMemberUser,
    svc: ProjectService = Depends(service),
):
    svc.remove_member(
        project_id,
        actor.id,
        actor,
        leaving=True,
    )

    return Response(
        status_code=204,
    )


@router.delete(
    "/{project_id}/members/{user_id}",
    status_code=204,
)
def remove_member(
    project_id: uuid.UUID,
    user_id: uuid.UUID,
    actor: ProjectManagerUser,
    svc: ProjectService = Depends(service),
):
    svc.remove_member(
        project_id,
        user_id,
        actor,
    )

    return Response(
        status_code=204,
    )


@router.patch(
    "/{project_id}/members/{user_id}/config",
    response_model=MemberOut,
)
def set_member_config(
    project_id: uuid.UUID,
    user_id: uuid.UUID,
    payload: MemberConfigUpdate,
    actor: ProjectOwnerUser,
    svc: ProjectService = Depends(service),
):
    member = svc.set_member_config(
        project_id,
        user_id,
        payload.can_config,
        actor,
    )

    return member_out(member)