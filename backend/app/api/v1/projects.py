...
def create_project(payload: ProjectCreate, actor: CurrentUser, svc: ProjectService = Depends(service)): return svc.create(payload, actor)

@router.get("", response_model=ProjectPage)
def list_projects(actor: CurrentUser, db: DbSession, q: str | None = None, project_status: ProjectStatus | None = Query(None, alias="status"), page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), sort: str = "-created_at"):
    data, total = ProjectRepository(db).list(actor.id, actor.role == UserRole.ADMIN, q, project_status, page, size, sort)
    return {"data": data, "meta": {"page": page, "size": size, "total": total}}

@router.get("/{project_id}", response_model=ProjectOut)
def get_project(project_id: uuid.UUID, actor: CurrentUser, svc: ProjectService = Depends(service)):
    project = svc.require_project(project_id); svc.require_member(project_id, actor); return project

@router.put("/{project_id}", response_model=ProjectOut)
def update_project(project_id: uuid.UUID, payload: ProjectUpdate, actor: CurrentUser, svc: ProjectService = Depends(service)): return svc.update(project_id, payload, actor)

@router.patch("/{project_id}/close", response_model=ProjectOut)
def close_project(project_id: uuid.UUID, actor: CurrentUser, svc: ProjectService = Depends(service)): return svc.close(project_id, actor)
...
def add_member(project_id: uuid.UUID, payload: MemberCreate, actor: CurrentUser, svc: ProjectService = Depends(service)):
...
def change_member_role(project_id: uuid.UUID, user_id: uuid.UUID, payload: MemberRoleUpdate, actor: CurrentUser, svc: ProjectService = Depends(service)):
...
def leave_project(project_id: uuid.UUID, actor: CurrentUser, svc: ProjectService = Depends(service)): svc.remove_member(project_id, actor.id, actor, leaving=True); return Response(status_code=204)
